    ...
    for order in orderbook.bids[::-1]:
        if 1000 * float(order.price) >= (1000 - 1000 * buy_price_no):
            if float(order.size) > 1:
                rounded_size = math.floor(float(order.size))
...
def place_orders(client, market, coin, size, expiration_time, strike, yes_token, no_token, lower_bound, upper_bound, probability_factor=1, three_decimals=False):
    while True:
        try:
            previous_orders = client.get_orders(FilterParams(market=market, ))

            try:
                probability = probability_factor * calculate_probability.find_probability(coin, expiration_time, strike)
            except:
            ...
            if three_decimals:
                ...
                if probability > low_price + 0.12:
                    buy_price_yes_limit = math.floor(probability - 0.1)
                else:
                    buy_price_yes_limit = math.floor(min((probability - lower_bound), low_price + 0.001001) * 1000) / 1000
                if 1 - probability > 1 - high_price + 0.12:
                    buy_price_no_limit = math.floor(1 - probability - 0.1)
                else:
                ...
                if probability > low_price + 0.12:
                    buy_price_yes_limit = math.floor((probability - 0.1) * 100) / 100
                else:
                    buy_price_yes_limit = math.floor(min((probability - lower_bound), low_price + 0.01001) * 100) / 100
                if 1 - probability > 1 - high_price + 0.12:
                    buy_price_no_limit = math.floor((1 - probability - 0.1) * 100) / 100
                else:
